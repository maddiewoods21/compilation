...
function evade(evt) {
    ...
    var delta = dist.normal().multeq(closest).sub(dist),
        newCorner = {left: corner.left + delta.x * 1.5, top: corner.top + delta.y * 1.5};
...
$(function () {
    $('.two').wrap('<span class="bumperTwo" />')

    $('.bumperTwo').bind('mouseover', beginEvade);
    $('.bumperTwo').bind('mouseout', endEvade);
})
